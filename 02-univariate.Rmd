...
kable(table1(~age + sex + meth_ingest + meth_user + license_status + education + ethnicity + area_live, data = x)) %>% 
...
ma.df <- screened %>% filter(meth_ingest == "Yes")
...
kable(table1(~meth_use_age + meth_use_at_peak + meth_last_12_months + meth_recent_use + meth_use_extreme , data = ma.df)) %>% 
  kable_styling("striped") %>% 
  scroll_box(height = "500px")
```
