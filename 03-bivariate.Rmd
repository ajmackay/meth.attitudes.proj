...
## SDS Scale
Bivariate relationship between SDS and DDDI and STAXI and stuff
```{r SDS.bivar}
screened %>% 
  ggplot(aes(x = meth_SDS, y = DDDI_Total)) +
  geom_point()

screened %>% 
  ggplot(aes(x = meth_SDS, y = T_Ang_Total)) +
  geom_point()
...
## Drug Use
### Alcohol (AUDIT-C)
```{r alcohol.bivar, echo = F, warning=F}
plot.bivar(screened, AUDIT_Total, DDDI_Total, meth_ingest) +
  labs(x = "AUDIT-C Total", y = "DDDI Total", col = "MA Use") +
  ggtitle("DDDI Total with AUDIT-C")
